...
WITH cte_json
AS (SELECT * FROM json.`${DA.paths.kafka_events}`) --query
SELECT * FROM cte_json --subquery

-- COMMAND ----------

WITH cte_json_count
AS (SELECT count(1) FROM json.`${DA.paths.kafka_events}`)
SELECT * FROM cte_json_count
